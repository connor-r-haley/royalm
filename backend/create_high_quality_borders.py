...
def simplify_geometry_high_quality(geometry, tolerance=0.1):
    """Simplify geometry with high quality preservation"""
...
def create_high_quality_borders():
    """Create high-quality borders with better detail"""
    try:
        # Load the real borders
        with open('wwiii-countries-real.json', 'r') as f:
            data = json.load(f)
        
        print("Creating high-quality borders...")
        ...
        for feature in data['features']:
            ...
            # Simplify with much higher quality (smaller tolerance)
            high_quality_geometry = simplify_geometry_high_quality(original_geometry, tolerance=0.05)
            
            # Count high-quality points
            if high_quality_geometry['type'] == 'Polygon':
                for ring in high_quality_geometry['coordinates']:
                    total_points_after += len(ring)
            elif high_quality_geometry['type'] == 'MultiPolygon':
                for polygon in high_quality_geometry['coordinates']:
                    for ring in polygon:
                        total_points_after += len(ring)
            
            # Create high-quality feature
            high_quality_feature = {
                'type': 'Feature',
                'properties': feature['properties'],
                'geometry': high_quality_geometry
            }
            high_quality_features.append(high_quality_feature)
        
        # Create high-quality GeoJSON
        high_quality_data = {
            'type': 'FeatureCollection',
            'features': high_quality_features
        }
        
        # Save high-quality version
        with open('wwiii-countries-high-quality.json', 'w') as f:
            json.dump(high_quality_data, f, indent=2)
        
        print(f"High-quality borders created!")
        ...
        print(f"File size: {len(json.dumps(high_quality_data)) / 1024 / 1024:.1f} MB")
